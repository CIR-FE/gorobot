 ...
 * A TCP which is approximately at the target position will be created, and must be set to the robot
 * before the poses can be used.
 *
 ...
 * @param   initialPose  The initial pose around which the ring will be created. The sensor is
 * assumed to be looking straight down at this position.
 * @param   poseArray    A pointer to the array of points. Array should be of size poseCount.
 * @return               kStatus
 */
GoDx(void) EyeOnHandCalibrationPoses(kSize poseCount, k64f maxAngle, Matrix initialPose,
                                     Matrix *poseArray);
