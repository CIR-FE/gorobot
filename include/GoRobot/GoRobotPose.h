...
GoDx(void) PoseValuesFromMatrix(const Matrix& matrix, PoseType pose_type, AngleUnits angle_units,
                                Units units, double& x, double& y, double& z, double& rx,
                                double& ry, double& rz);
GoDx(Matrix) PoseValuesToMatrix(PoseType pose_type, AngleUnits angle_units, Units units, double x,
 ...
 */
template <PoseType pose_type = PoseType::XYZRPY, AngleUnits angle_units = AngleUnits::DEGREES,
          Units units = Units::MILIMETERS>
class Pose
{
 private:
 public:
